...
def generer_graph(filenames, path):
    ...
    for i, filename in enumerate(filenames):
        ...
        if filtre == "pas":
            ...
            epaisseur = int(filtre[index_epp+1:])*0.002540
        ...
        somme = np.sum(data_reduit) / live_time
    ...
    for i in range(len(array_dict)):
        N0_tuple = (somme0, 0) # Valeur, incert
        Nt_tuple = (somme_comptes_array[i], incert_y_array[i])
        (atau, atau_incert) = a_tau(N0_tuple, Nt_tuple, A_array[i], rho_array[i], epaisseur_array[i])
        atau_array[i] = atau
    ...
    donnees_atau = [atau_array, atau_incert_array]
    donnees_Z = Z_array
    ...
    fig = plt.scatter(donnees_Z, donnees_atau[0], label=f"Section efficace a_tau")
